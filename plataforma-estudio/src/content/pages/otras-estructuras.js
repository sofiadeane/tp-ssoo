...
export default {
  intro: [
    {
      type: 'p',
      text: 'Todo SO necesita mantener el estado de lo que administra en estructuras dedicadas — clásicamente agrupadas en 4 categorías: tablas de procesos ({{g:PCB|la "ficha" que el sistema arma para cada proceso, con todos sus datos: estado, prioridad, registros, etc.}}), tablas de memoria ({{g:segmentos|un pedazo contiguo de memoria física asignado a un proceso}}/páginas y espacio libre), tablas de E/S (qué dispositivo está haciendo qué) y tablas de archivos (no aplica en este TP, ya que `memory_stick` no es un filesystem).',
    },
    {
      type: 'p',
      text: 'Este TP tiene una estructura por cada una de las primeras tres categorías, repartidas entre `KS` y `KM` según a quién le corresponde esa responsabilidad. Esta página funciona como catálogo: recorre esas estructuras a través de todos los módulos.',
    },
    {
      type: 'callout',
      tone: 'info',
      title: 'Qué NO cubre esta página',
      text: 'La sincronización ({{g:mutex|un candado que solo un proceso puede tener a la vez}}, semáforos, herencia de prioridad) ya se explica en la página de Sincronización. Acá se cubren las estructuras de datos en sí, independientemente de cómo se protegen.',
    },
  ],
  ...
  concepts: [
    {
      type: 'p',
      text: 'El PCB (Process Control Block) es la estructura de control central: representa a cada proceso en `KS`. En este TP es el `t_pcb`, definido en `kernel_scheduler/src/scheduler.h:11-27`.',
    },
    {
      type: 'code',
      text: `typedef struct {
    ...
    },
    {
      ...
      items: [
        ...
        'No hay un PCB separado del lado de `KM`: `KM` tiene su propia estructura equivalente para lo que le corresponde (`t_proceso_km`, ver sección de detalles).',
      ],
    },
  ],
  ...
  howItWorks: [
    {
      type: 'p',
      text: 'Todas las colas de procesos viven en `KS` y están implementadas con `t_queue*` de so-commons-library (`commons/collections/queue.h`) — ninguna es una estructura propia reimplementada. No se usa `t_dictionary` en ningún módulo del repo, aunque `kernel_scheduler/src/main.c` incluye el header sin llegar a usarlo.',
    },
    {
      type: 'table',
  ...
  stepByStep: [
    ...
    },
    {
      ...
      rows: [
        ['Procesos (PCB)', '`KS`', '`t_pcb`', '`lista_todos_pcb` + colas por estado'],
        ['Segmentos activos/suspendidos', '`KM`', '`t_segmento` / `t_segmento_suspendido`', '`t_proceso_km.segmentos[_suspendidos]`'],
        ['Espacio libre', '`KM`', '`t_hueco`', '`lista_huecos`'],
        ['Memory Sticks conectados', '`KM`', '`t_memory_stick_info`', '`memory_sticks`'],
        ['Dispositivos IO', '`KS`', '`t_io_modulo` / `t_io_pedido`', '`lista_modulos_io` + cola propia por dispositivo'],
        ['Mutex de usuario', '`KS`', '`t_mutex_simulado`', '`lista_mutexes`'],
  ...
  details: [
    ...
    },
    {
      ...
      tone: 'info',
      title: '`t_hueco`',
      text: '`t_hueco` (`kernel_memory.h:54-57`) representa un {{g:hueco|un espacio libre en la memoria, todavía sin asignar a nadie}}: es `{base, limite}`, el espacio libre contiguo guardado en `lista_huecos`, la estructura que usa `encontrar_hueco` para first/best/worst-fit.',
    },
    {
      type: 'callout',
      tone: 'info',
    ...
    },
    {
      ...
      tone: 'info',
      title: '`t_cpu`',
      text: '`t_cpu` (`utils.h:107-113`) es `{id, socket, socket_interrupt, libre, cpu_socket}`: cada CPU conectada al sistema, si está libre u ocupada, y sus dos sockets (ejecución + interrupciones). Vive en `cpus_conectadas` (`t_list*`, compartida entre `KS` y `KM` vía `utils.c`), protegida por `mutex_cpus`. El semáforo `sem_cpus_libres` (contador de CPUs libres) es la otra cara de esta misma tabla.',
    },
    {
      type: 'p',
      text: 'Extra: `t_info_finalizado` (`kernel_scheduler/src/main.c:106-109`) es `{pid, motivo}` — {{f:un log acumulativo|No es una tabla de control en el sentido clásico, pero sí una estructura de estado del sistema: un log acumulativo que se imprime completo cada vez que termina un proceso (vía `registrar_fin_proceso`, :113-133), protegido por `mutex_lista_finalizados`. Útil para mostrar en la defensa como evidencia de qué terminó y por qué.}}.',
    },
  ],
  ...
  summary: [
    {
      type: 'ul',
      items: [
        'El PCB (`t_pcb`) es la tabla de procesos: vive en `KS`, en `lista_todos_pcb` más las colas por estado.',
        ...
        'La memoria se administra por segmentación, no paginación: `t_segmento`, `t_segmento_suspendido` y `t_hueco` viven en `KM`, junto con `t_memory_stick_info` para los Memory Sticks conectados.',
        '`t_io_modulo`/`t_io_pedido` representan los dispositivos IO y sus pedidos, del lado de `KS`.',
        ...
        '`t_cpu` es la tabla de CPUs conectadas, compartida entre `KS` y `KM`.',
        'Tabla resumen de quién es dueño de qué: Procesos → `KS` (`t_pcb`); Segmentos → `KM` (`t_segmento`/`t_segmento_suspendido`); Espacio libre → `KM` (`t_hueco`); Memory Sticks → `KM` (`t_memory_stick_info`); Dispositivos IO → `KS` (`t_io_modulo`/`t_io_pedido`); Mutex de usuario → `KS` (`t_mutex_simulado`); CPUs → compartida (`t_cpu`).',
      ],
    },
  ],
}
