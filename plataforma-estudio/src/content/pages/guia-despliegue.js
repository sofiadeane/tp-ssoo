...
export default {
  intro: [
    {
      type: 'p',
      text: 'Esta guía es el runbook de despliegue del Grupo R2D2 (1C 2026): describe cómo preparar la VM de la cátedra (UTNSO), clonar el proyecto con so-deploy, y los comandos exactos de terminal para correr cada prueba integradora antes del final.',
    },
    {
      type: 'callout',
      tone: 'info',
  ...
  concepts: [
    ...
    {
      type: 'p',
      text: 'Conexión por SSH desde la VM: obtener la IP local de la VM y conectarse desde la terminal.',
    },
    {
      type: 'code',
  ...
  howItWorks: [
    {
      type: 'p',
      text: 'La batería de pruebas de la cátedra se organiza en 6 pruebas, cada una levantando una combinación distinta de módulos y configs. Todas comparten el mismo orden lógico de encendido: kernel_memory → kernel_scheduler → memory_stick → swap → io → cpu.',
    },
    {
      type: 'table',
      ...
      rows: [
        ...
        ['2. Planificación Corto Plazo (PCP)', 'Algoritmo de planificación de corto plazo corriendo en loop continuo (PCP.prc).'],
        ...
        ['4. Planificación Mediano Plazo (PMP)', 'Planificación de mediano plazo con varios memory_stick y tres instancias de IO (SLEEP, STDIN, STDOUT) en simultáneo.'],
        ['5. Herencia de Prioridades (PHP)', 'Mecanismo de herencia de prioridades entre procesos (PHP.prc).'],
  ...
  stepByStep: [
    ...
    },
    {
      ...
      rows: [
        ['TERMINAL 1', 'kernel_memory', 'cd ~/so-deploy/tp-2026-1c-R2D2/kernel_memory && ./bin/kernel_memory kernel_memory.config'],
        ['TERMINAL 2 (P1)', 'kernel_scheduler', 'cd ~/so-deploy/tp-2026-1c-R2D2/kernel_scheduler && ./bin/kernel_scheduler scheduler.config /home/utnso/so-deploy/tp-2026-1c-R2D2/plug-n-pray-pruebas/PLANI_PRE_0.prc'],
    ...
    },
    {
      ...
      headers: ['Terminal', 'Módulo', 'Comando'],
      rows: [
        ['TERMINAL 2 (P2)', 'kernel_scheduler', 'cd ~/so-deploy/tp-2026-1c-R2D2/kernel_scheduler && ./bin/kernel_scheduler scheduler.config /home/utnso/so-deploy/tp-2026-1c-R2D2/plug-n-pray-pruebas/MEMORIA_PRE_0.prc'],
    ...
    },
    {
      ...
      headers: ['Terminal', 'Módulo', 'Comando'],
      rows: [
        ['TERMINAL 1', 'kernel_memory', 'cd ~/so-deploy/tp-2026-1c-R2D2/kernel_memory && ./bin/kernel_memory kernel_memory2.config'],
        ['TERMINAL 6', 'kernel_scheduler', 'cd ~/so-deploy/tp-2026-1c-R2D2/kernel_scheduler && ./bin/kernel_scheduler scheduler2.config /home/utnso/so-deploy/tp-2026-1c-R2D2/plug-n-pray-pruebas/PCP.prc'],
    ...
    },
    {
      ...
      headers: ['Terminal', 'Módulo', 'Comando'],
      rows: [
        ['TERMINAL 1', 'kernel_memory', 'cd ~/so-deploy/tp-2026-1c-R2D2/kernel_memory && ./bin/kernel_memory kernel_memory_3_best.config'],
        ['TERMINAL 9', 'kernel_scheduler', 'cd ~/so-deploy/tp-2026-1c-R2D2/kernel_scheduler && ./bin/kernel_scheduler scheduler3.config /home/utnso/so-deploy/tp-2026-1c-R2D2/plug-n-pray-pruebas/PLANI_MEM.prc'],
    ...
    },
    {
      ...
      headers: ['Terminal', 'Módulo', 'Comando'],
      rows: [
        ['TERMINAL 1', 'kernel_memory', 'cd ~/so-deploy/tp-2026-1c-R2D2/kernel_memory && ./bin/kernel_memory kernel_memory4.config'],
        ['TERMINAL 2', 'kernel_scheduler', 'cd ~/so-deploy/tp-2026-1c-R2D2/kernel_scheduler && ./bin/kernel_scheduler scheduler4.config /home/utnso/so-deploy/tp-2026-1c-R2D2/plug-n-pray-pruebas/PMP.prc'],
    ...
    },
    {
      ...
      headers: ['Terminal', 'Módulo', 'Comando'],
      rows: [
        ['TERMINAL 1', 'kernel_memory', 'cd ~/so-deploy/tp-2026-1c-R2D2/kernel_memory && ./bin/kernel_memory kernel_memory5.config'],
        ['TERMINAL 7', 'kernel_scheduler', 'cd ~/so-deploy/tp-2026-1c-R2D2/kernel_scheduler && ./bin/kernel_scheduler scheduler5.config /home/utnso/so-deploy/tp-2026-1c-R2D2/plug-n-pray-pruebas/PHP.prc'],
        ['TERMINAL 2', 'ms1', 'cd ~/so-deploy/tp-2026-1c-R2D2/memory_stick/ms1 && ../bin/memory_stick memory_stick.config 16'],
  ...
  details: [
    {
      ...
      text: 'Entre la Parte 1 y la Parte 2 de la Prueba Base hay que reiniciar todos los módulos del sistema antes de continuar. La Parte 2 reutiliza el mismo kernel_scheduler y el mismo io (STDIN_STDOUT), pero con otro .prc (MEMORIA_PRE_0).',
    },
    {
      type: 'callout',
      tone: 'info',
      title: 'Segunda corrida de la Prueba de Memoria',
      text: 'Para comparar estrategias de asignación hay que reiniciar todos los procesos cambiando la variable ALLOCATION_STRATEGY a WORST en kernel_memory.config, y volver a levantar kernel_memory con kernel_memory_3_worse.config.',
    },
  ],
  ...
  summary: [
    {
      ...
      items: [
        ...
        'Orden de encendido en todas las pruebas: kernel_memory → kernel_scheduler → memory_stick → swap → io → cpu.',
        'Prueba Base: correr Parte 1, reiniciar todo, correr Parte 2 (mismo kernel_scheduler/io, distinto .prc).',
