...
export default {
    ...
    },
    {
      ...
      rows: [
        [
          ...
          'Casi todos los módulos en la etapa de estabilización. Compactación de memoria en `KM`, {{g:deadlock|un embotellamiento donde dos o más quedan esperándose mutuamente para siempre}} de {{g:mutex|un candado que solo un proceso puede tener a la vez}} por {{g:herencia de prioridad|cuando un proceso "presta" temporalmente su prioridad alta a otro que tiene bloqueado un recurso que necesita}}, bug de {{g:EINTR|un error que indica que una espera fue interrumpida por una señal del sistema operativo antes de completarse}} en esperas de semáforo/sleep, bugs de comunicación `scheduler↔io↔memstick`, y coescribió el módulo `CPU` inicial junto con Rominna Aquino.',
        ],
        [
          'Priscila Zárate (priscilazarate)',
          '`KS`: manejo de hilos, identificador de módulos `IO`, planificación de largo y mediano plazo; también worst/best-fit en `KM` y un refactor temprano de `utils` (mover `aceptar_conexiones` a la librería compartida).',
        ],
        [
          'Florencia Molle',
        ...
        [
          ...
          'Creó el módulo `KM` desde cero y, del lado de `KS`, el planificador de corto plazo y la primera versión de colas multinivel con herencia de prioridad; también revisó/mergeó las ramas de `kernel-scheduler` y colas multinivel de sus compañeros.',
        ],
        [
  ...
  howItWorks: [
    ...
    },
    {
      type: 'ul',
      items: [
        'Cada módulo arranca por separado (abril): los 4 módulos "de proceso" (`CPU`, `IO` y `SWAP`, `KS`, `KM`) nacieron como commits independientes de cada persona, todavía sin comunicación completa entre ellos.',
        'Planificación completa y MMU (mayo-junio): colas multinivel con herencia de prioridad, planificador de corto y largo plazo, worst/best-fit en `KM`, y la MMU con soporte para varios Memory Sticks.',
