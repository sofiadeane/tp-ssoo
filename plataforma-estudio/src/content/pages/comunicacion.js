...
export default {
  intro: [
    {
      type: 'p',
      text: 'Los 6 módulos del sistema son procesos independientes, potencialmente corriendo en máquinas distintas: no comparten memoria ni variables. La única forma de comunicarse es IPC, y este TP elige sockets TCP con un modelo cliente-servidor — cada módulo puede levantar su propio servidor (para que otros se conecten) y a la vez actuar como cliente de otros módulos.',
    },
    {
      type: 'callout',
      tone: 'info',
      title: 'Idea central',
      text: 'TCP garantiza un stream de bytes confiable y en orden, pero no garantiza mensajes delimitados. Por eso el sistema define su propio protocolo de aplicación sobre TCP — el "paquete" — para saber dónde empieza y termina cada mensaje.',
    },
  ],
  ...
  concepts: [
    ...
    },
    {
      ...
      items: [
        ...
        'op_code: un enum de ~30 códigos (INICIAR_PROCESO, PEDIR_INSTRUCCIONES, LEER_MEMORIA, EJECUTAR_PROCESO, SYSCALL, MUTEX_LOCK, SEG_FAULT, ESCRIBIR_BLOQUE_SWAP, SUSPENDER_PROCESO, CREAR_SEGMENTO, INICIAR_COMPACTACION, etc.). Es la "identidad" del mensaje: quien lo recibe hace un switch sobre este valor.',
      ],
    },
    {
      type: 'callout',
    ...
    {
      type: 'p',
      text: 'Dentro del stream, cada valor agregado con agregar_a_paquete (utils.c:153-161) queda codificado como TLV simplificado — tamaño + valor, sin un campo de "tipo" explícito. Quien lee tiene que saber de antemano en qué orden vienen los campos:',
    },
    {
      type: 'code',
  ...
  howItWorks: [
    ...
    },
    {
      ...
      rows: [
        ...
        ['esperar_conexion / aceptar_conexiones(socket_servidor)', 'utils.c:59-73, 294-365', 'accept() de una conexión. aceptar_conexiones además hace el handshake de identificación y lanza un hilo por cliente (pthread_create + pthread_detach).'],
    ...
    },
    {
      ...
      items: [
        ...
        'memory_stick: además de registrarse como cliente ante KM, levanta su propio servidor para que las CPUs se conecten directo.',
  ...
  details: [
    {
      ...
      text: 'El que se conecta manda un int crudo con su t_modulo (CPU=1, KS=2, SWAP=3, MEMSTICK=4, IO=5, KM=6). Según el tipo, puede mandar un segundo dato: CPU manda su id, MEMSTICK e IO mandan su tamanio (en el caso de IO, ese "tamaño" es en realidad el largo del nombre del dispositivo que viene después). El servidor responde ok=1 desde manejar_cliente.',
    },
    {
      type: 'callout',
      tone: 'info',
      title: 'Handshake — mecanismo B: verificar_conexion (utils.c:76-99)',
      text: 'Usado del lado cliente hacia Kernel Memory: manda su t_modulo y espera una confirmación int por recv. Es funcionalmente parecido al mecanismo A, pero está implementado por separado (nombre de función distinto, sin pasar por aceptar_conexiones). Son dos mecanismos de identificación distintos conviviendo en el mismo sistema.',
    },
    {
      type: 'callout',
      tone: 'info',
      title: 'Caso particular de IO',
      text: 'Después del handshake genérico, IO manda además el nombre lógico del dispositivo (ej. "STDIN_STDOUT", "IMPRESORA") como length-prefixed string (io/src/main.c:75-77). kernel_scheduler usa ese nombre para registrar el módulo con registrar_modulo_io(socket, nombre) — así sabe a qué IO mandarle cada pedido cuando el proceso pide, por ejemplo, "STDIN_STDOUT".',
    },
    {
      type: 'callout',
      tone: 'info',
  ...
  commonErrors: [
    {
      ...
      text: 'Tres canales usan send/recv con opcodes y datos crudos, SIN el envoltorio t_paquete/TLV: CPU ↔ memory_stick (lectura/escritura directa de memoria: opcode LEER_MEMORIA/ESCRIBIR_MEMORIA + dirección + tamaño, todo a mano), kernel_memory ↔ swap (ESCRIBIR_BLOQUE_SWAP/LEER_BLOQUE_SWAP + número de bloque + bytes del bloque, crudo), y kernel_memory ↔ memory_stick cuando KM necesita acceder a memoria directamente (mismo estilo crudo).',
    },
    {
      type: 'p',
      text: 'Es una trampa típica de examen: si preguntan "¿todo pasa por el mismo protocolo de paquetes?", la respuesta correcta es no. Los canales de alto volumen de datos (memoria física, bloques de swap) usan un protocolo más liviano ad-hoc; los canales de control/decisiones (KS↔CPU, KS↔KM) usan el t_paquete genérico. Es una decisión de diseño razonable (evita el overhead de armar/parsear TLV en transferencias grandes y frecuentes), aunque también se puede leer como una inconsistencia de estilo del código.',
    },
  ],
  ...
  summary: [
    {
      ...
      items: [
        ...
        'IO es un caso especial: además del handshake genérico, manda su nombre lógico de dispositivo para que kernel_scheduler lo registre.',
