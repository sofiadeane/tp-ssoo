...
export default {
  intro: [
    {
      type: 'p',
      text: 'Un sistema operativo clásico organiza la planificación de procesos en tres niveles. kernel_scheduler (KS) es el módulo más denso del TP porque implementa los tres a la vez, cada uno en su propio hilo.',
    },
    {
      type: 'table',
    ...
    },
    {
      ...
      tone: 'info',
      title: 'Idea central',
      text: 'KS es el orquestador: mantiene el PCB de cada proceso, decide quién ejecuta y cuándo, e intermedia entre CPU, IO y Kernel Memory. Es, ademas, dueño de los mutex simulados que puede pedir el proceso de usuario.',
    },
  ],
  ...
  concepts: [
    {
      type: 'p',
      text: 'Todo proceso tiene un PCB (t_pcb) que guarda su estado, prioridad, program counter, quantum restante, la cola de la que vino y el motivo de su último desalojo.',
    },
    {
      type: 'ul',
      items: [
  ...
  howItWorks: [
    ...
    },
    {
      ...
      items: [
        ...
        'Si el algoritmo configurado es CMN (colas multinivel), recorre las colas en orden de prioridad estricta — índice 0 es la más prioritaria — y toma el primer proceso de la primera cola no vacía. Nunca mira una cola de menor prioridad si hay algo esperando en una más prioritaria.',
        'Si el algoritmo es FIFO o RR (una sola cola), simplemente saca el primero de esa cola.',
    ...
    {
      type: 'p',
      text: 'Cuando una cola usa Round Robin, KS lanza un hilo temporizador que, al vencer el quantum, manda una interrupción — pero solo si el proceso todavía sigue en EXEC. Ese chequeo evita interrumpir a un proceso que ya terminó o se bloqueó por su cuenta justo antes de que venciera el quantum.',
    },
    {
      type: 'p',
  ...
  stepByStep: [
    ...
    },
    {
      ...
      items: [
        ...
        'Si sigue vigente, pide a Kernel Memory que suspenda al proceso y lo mueve a SUSP_BLOCK.',
        'Por otro lado, la desuspensión se dispara ante tres eventos: Kernel Memory libera memoria, se conecta un Memory Stick nuevo, o termina una compactación.',
        ...
        'Le pide a Kernel Memory que lo desuspenda; si hay espacio, lo pasa a READY.',
      ],
    },
  ],
  ...
  details: [
    {
      ...
      text: 'Los mutex simulados que puede pedir el proceso de usuario implementan herencia de prioridad para evitar inversión de prioridades: si un proceso de baja prioridad tiene tomado un mutex que necesita uno de alta prioridad, se le eleva temporalmente la prioridad al que lo tiene tomado para que libere el recurso antes. El detalle completo (incluido un fix de deadlock histórico) vive en el informe de sincronización — acá alcanza con saber que existe y por qué.',
    },
    {
      type: 'callout',
      tone: 'info',
      title: 'Compactación y coordinación con Kernel Memory',
      text: 'Cuando Kernel Memory no encuentra un hueco contiguo para asignar memoria, dispara una compactación y avisa a KS. KS entonces desaloja todas las CPUs activas salvo la que originó el pedido (para no generar un deadlock si hay una sola CPU: esa CPU está bloqueada esperando su propia respuesta y no puede además recibir una interrupción) y bloquea el planificador de corto plazo hasta que Kernel Memory avisa que terminó.',
    },
    {
      type: 'callout',
      tone: 'info',
      title: 'Por qué la desuspensión corre en un hilo aparte',
      text: 'El único hilo que lee respuestas de Kernel Memory es hilo_escucha_memoria. Si la desuspensión se llamara inline desde ahí al recibir el aviso de memoria disponible, y esa lógica necesitara a su vez pedirle algo a Kernel Memory y esperar la respuesta, el hilo terminaría esperándose a sí mismo — deadlock. Por eso se dispara en un hilo aparte.',
    },
    {
      type: 'p',
  ...
  summary: [
    {
      type: 'ul',
      items: [
        'kernel_scheduler implementa los tres niveles clásicos de planificación (largo, mediano y corto plazo), cada uno en su propio hilo.',
        ...
        'La compactación de memoria pausa el planificador de corto plazo y desaloja CPUs para coordinarse con Kernel Memory.',
