...
export default {
  intro: [
    {
      ...
      text: '`memory_stick` simula memoria física (RAM) de un "pendrive"/placa enchufable, no un {{g:filesystem|un sistema de archivos, con carpetas, nombres y metadata — memory_stick NO tiene nada de esto, es memoria física pura}}. No hay archivos, directorios, bloques con metadata ni journaling: es un único {{g:buffer|un bloque de memoria reservado para guardar datos temporalmente}} plano de bytes. Si preguntan por compactación o fragmentación de un memory stick, esa lógica vive en `KM`, no acá.',
    },
    {
      type: 'p',
    ...
    {
      type: 'p',
      text: 'No contienen datos ni son particiones de un filesystem: son solo configuración para simular memoria física expandible/"plug-n-play" — coherente con el mensaje `NUEVA_MEMORIA_DISPONIBLE` que `KM` le manda a `KS` cuando se conecta un nuevo stick.',
    },
  ],
  ...
  concepts: [
    ...
    },
    {
      ...
      items: [
        ...
        'Sin bloques, sin metadata de archivos, sin journaling: toda la "inteligencia" de memoria (compactación, fragmentación, segmentos) vive en `KM`.',
  ...
  howItWorks: [
    ...
    {
      type: 'p',
      text: 'Después se registra ante `KM`: conecta, manda `tipo=MEMSTICK`, tamaño, y además su propia IP y puerto de escucha — así `KM` puede reenviárselos después a las CPUs para que se conecten directo con el stick.',
    },
    {
      type: 'p',
  ...
  stepByStep: [
    ...
    },
    {
      ...
      rows: [
        ['`LEER_MEMORIA`', '`dir` (uint32) + `tam` (uint32)', 'Duerme `MEMORY_DELAY` ms, toma {{g:mutex|un candado que solo un proceso puede tener a la vez}} `mutex_memoria`, `send(&memoria[dir], tam)`.'],
  ...
  summary: [
    {
      type: 'ul',
      items: [
        '`memory_stick` es memoria física pura: un buffer plano de bytes, sin filesystem, sin metadata ni bloques — esa lógica vive en `KM`.',
        ...
        'Se registra ante `KM` mandando su propia IP/puerto, y además levanta un servidor propio para que las CPUs se conecten directo, sin pasar por `KM`.',
